...
from py_hcl.firrtl_ir.type_checker import check, ConnectTypeChecker
from ..utils import serialize_stmt_equal


def test_connect_basis():
    cn = Connect(n("a", uw(8)), n("b", uw(8)))
    assert ConnectTypeChecker.check(cn)
    assert check(cn)
    ...
    assert ConnectTypeChecker.check(cn)
    assert check(cn)
    serialize_stmt_equal(cn, 'a <= UInt<8>("14")')

    cn = Connect(n("a", sw(8)), s(-20, w(8)))
    assert ConnectTypeChecker.check(cn)
    assert check(cn)
    serialize_stmt_equal(cn, 'a <= SInt<8>("-14")')


def test_connect_type_wrong():
    cn = Connect(n("a", uw(8)), n("b", sw(8)))
    assert not ConnectTypeChecker.check(cn)
    assert not check(cn)
