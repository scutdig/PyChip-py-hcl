...
class VerilogOptimize(Pass):
    def run(self, c: Circuit):
        ...
        def verilog_optimize_e(expr: Expression, node_map: Dict[str, Statement], filter_nodes: set) -> Expression:
            if isinstance(expr, (UIntLiteral, SIntLiteral)):
                ...
                if en in node_map:
                    filter_nodes.add(en)
                    return verilog_optimize_e(node_map[en].value, node_map, filter_nodes)
                else:
                    return expr
                ...
                return Mux(
                    ...
                    verilog_optimize_e(expr.tval, node_map, filter_nodes),
                    verilog_optimize_e(expr.fval, node_map, filter_nodes), expr.typ)
            elif isinstance(expr, ValidIf):
                return ValidIf(
                    verilog_optimize_e(expr.cond, node_map, filter_nodes),
                    verilog_optimize_e(expr.value, node_map, filter_nodes), expr.typ)
            elif isinstance(expr, DoPrim):
                args = list(map(lambda arg: verilog_optimize_e(arg, node_map, filter_nodes), expr.args))
                return DoPrim(expr.op, args, expr.consts, expr.typ)
            else:
                return expr

        def verilog_optimize_s(stmt: Statement, node_map: Dict[str, Statement], filter_nodes: set) -> Statement:
            if isinstance(stmt, Block):
                node_map = {**node_map ,**{sx.name: sx for sx in stmt.stmts if auto_gen_node(sx)}}
                cat_stmts = []
                for sx in stmt.stmts:
                    if isinstance(sx, Connect):
                        cat_stmts.append(Connect(verilog_optimize_e(sx.loc, node_map, filter_nodes), verilog_optimize_e(sx.expr, node_map, filter_nodes),
                        sx.info, sx.blocking, sx.bidirection, sx.mem))
                    elif isinstance(sx, DefNode):
                        cat_stmts.append(DefNode(sx.name, verilog_optimize_e(sx.value, node_map, filter_nodes), sx.info))
                    elif isinstance(sx, Conditionally):
                        cat_stmts.append(Conditionally(verilog_optimize_e(sx.pred, node_map, filter_nodes), verilog_optimize_s(sx.conseq, node_map, filter_nodes),
                        verilog_optimize_s(sx.alt, node_map, filter_nodes), sx.info))
                    else:
                        cat_stmts.append(sx)
                ...
                return Conditionally(verilog_optimize_e(stmt.pred, node_map, filter_nodes), verilog_optimize_s(stmt.conseq, node_map, filter_nodes), 
                verilog_optimize_s(stmt.alt, node_map, filter_nodes), stmt.info)
            else:
                return stmt
